...
Shared/
  ...
  Screens/                  Home/Login/ServerSetup/Search/PrivacySecurity, MediaLibraryScreen (hosted in MovieLibraryScreen.swift) + LibraryHeroSection/LibraryPosterCard/LibrarySortFilterSheet, MediaDetailScreen + MediaDetail* siblings, VideoPlayerView, NativeVideoPresenter, HLSManifestLoader, PlayLink
    VideoPlayer/            PlaybackReporter, SkipSegmentController, SleepTimerController, ChapterController, EndOfSeriesOverlayController, RemoteCommandController, NowPlayingInfoController, VLCStreamPresenter (covers online iOS+tvOS via stream init AND offline iOS via second init — same HUD class for both), CinemaxStreamProxy (StreamTransportPolicy IPv6 probe + loopback HTTP→URLSession proxy for broken-IPv6 servers), TrickplayController (scrub-preview tiles), NextUpCountdownView (outro autoplay card); shared extractions: PlayerTimeFormat (HH:MM:SS), PlayerEngineSurface (SwiftVLC host), PlayerTransportViews (TVScrubBar/ChapterChip/PassthroughView)
...
Widgets/CinemaxWidget/      (iOS) Continue-Watching WidgetKit extension — dependency-free (no CinemaxKit)
...
- **Targets**: `CinemaxWidget` (iOS WidgetKit, Continue Watching) and `CinemaxTopShelf` (tvOS Top Shelf). Both embed via target dependencies in `project.yml`; all four targets share App Group `group.com.cinemax.shared` (entitlements generated by xcodegen).
- **RULE — extensions do NOT link CinemaxKit** (widget memory budget; SDK pulls Nuke + generated entities). They read the session snapshot the app publishes via `ExtensionSessionBridge` (CinemaxKit/Persistence — App Group `UserDefaults`, key `extension.session`) and talk to `/UserItems/Resume` + image endpoints directly (`JellyfinLite` in the widget, inline in `ContentProvider`). **Keep the suite/key/JSON shape in sync across all three copies.** Publish hook: `AppState.refreshCurrentUser()` (covers login / switch / restore) + clear in `logout()`.
- **Deep links**: `cinemax://item/{id}` (scheme registered in both Info.plists). `AppNavigation.onOpenURL` → `AppState.handleDeepLink` → `pendingDeepLinkItemId`; `MainTabView` switches to the Home tab (drops the link if a custom menu has no Home); `HomeScreen` consumes it via a screen-root `navigationDestination(item:)` (lazy-container RULE applies).
...
- WidgetKit `TimelineProvider` witnesses must declare completions `@escaping @Sendable` to match the SDK or strict concurrency rejects the `Task` hop.
