...
final class HLSManifestLoader: NSObject, AVAssetResourceLoaderDelegate, @unchecked Sendable {
    ...
    /// from an HTTP cache — the manifest is rewritten on every fetch and the
    /// session is torn down with the player. Matches the `urlCache = nil`
    /// discipline of `JellyfinAPIClient.fastFailSessionConfiguration` and
    /// `JellyfinLite.session`.
    ...
    func resourceLoader(
        ...
        Self.session.dataTask(with: URLRequest(url: realURL)) { data, response, error in
